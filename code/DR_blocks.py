...
class DiagonalwiseRefactorization(nn.Module):
    def __init__(self, in_channels, kernel_size, padding, stride=1, groups=1):
        ...
        self.mask = nn.Parameter(torch.Tensor(self.get_mask(in_channels, channels, kernel_size)), requires_grad=False)
        self.weight = nn.Parameter(torch.Tensor(in_channels, channels, kernel_size, kernel_size), requires_grad=True)
        torch.nn.init.xavier_uniform(self.weight.data)
        self.weight.data.mul_(self.mask.data)

    def get_mask(self, in_channels, channels, kernel_size):
        mask = torch.zeros(in_channels, channels, kernel_size, kernel_size)
    ...
    def forward(self, x):
        ...
        x = torch.nn.functional.conv2d(x, weight, bias=None, stride=self.stride, padding=self.padding, groups=self.groups)
        return x


    def DepthwiseConv2d(in_channels, padding, kernel_size=3,stride=1):
        ...
        return DiagonalwiseRefactorization(in_channels, kernel_size, padding, stride, groups)
...
class ConvNeXtBlock_V1(nn.Module):
    ...
    def __init__(self, ndim, conv, dim, layer_scale=1e-6, stochastic_depth_prob=0.0, layer_norm=None, k_size=7):
        ...
        print("DIAGONALWISE_REFACTORIZATION")

        self.block = nn.Sequential(
            DiagonalwiseRefactorization.DepthwiseConv2d(dim, kernel_size=k_size[0], padding=pad),
